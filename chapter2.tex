...
\begin{definition}
Sei $ u = \langle u_1, u_2, ..., u_n \rangle \in \mathbb{F}_{q}^{n}$, wobei $\forall i, j \in \lbrace 1, ..., n \rbrace \colon u_i \neq u_j$. Ferner sei $v = \langle v_1, v_2, ..., v_n \rangle \in \mathbb{F}_{q}^{n} \setminus \lbrace 0 \rbrace$. Der \textbf{verallgemeinerte Reed-Solomon-Code} $GRS_k(u, v)$ besteht nun aus der Menge der Codeworte
...
\end{definition}
Verallgemeinerte Reed-Solomon-Codes sind in der Regel nicht zyklisch, folglich entfällt die Möglichkeit, ein Generatorpolynom anzugeben. Die Codierung basiert entsprechend auch nicht mehr auf einer simplen Multiplikation mit dem Generatorpolynom, sondern mit einer Generatormatrix. Verallgemeinerte Reed-Solomon-Codes finden Verwendung im Ansatz von Niederreiter und werden daher auch im Folgenden wieder betrachtet. \\\\
...
\[\langle a_1, a_2, ..., a_n\rangle \cdot \langle b_1, b_2, ..., b_n \rangle = \sum\limits_{i=1}^{n}a_{i}b{i}\] 
