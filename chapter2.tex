...
Im Anhang \ref{CodingLst} ist die Implementierung des Kodiervorgangs eines verallgemeinerten Reed-Solomon-Codes über endlichen Körpern in \textit{SageMath} dargestellt. Das Skript generiert aus den Stützstellen und Gewichten eine Generatormatrix für die nicht-systematische Kodierung. Die \textit{SageMath}-Funktion \texttt{rref()} führt die Zeilenreduktion anhand des Gauß-Jordan-Algorithmus aus und bildet die reduzierte Stufenform, die für die ersten $k$ Spalten die gewünschte Identitätsmatrix darstellt \parencite[vgl. ][]{SageMath}, \parencite[vgl. ][S. 56f]{Ling2004}.
