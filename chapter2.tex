...
\end{definition}
\begin{example}
Sei $a = \langle 0, 1, 2, 1 \rangle \in \mathbb{F}_{7}^{4}$. Dann ist $f(X) = 0 + 1 \cdot X + 2 \cdot X^2 + 1 \cdot X^3 \in \mathbb{F}_{7}[X]$ ein dazugehöriges Polynom. \\\\
...
\begin{definition}
Sei $C$ ein linearer $[n, k, d]_q$-Code mit Länge $n$, Dimension $k$ und Minimaldistanz $d$ über einem Körper $\mathbb{F}_q$. Ferner bezeichne die Operation $\circ$ das Skalarprodukt über dem Untervektorraum $\mathbb{F}_{q}^{n}$, das definiert ist durch
\[\langle a_1, a_2, ..., a_n\rangle \circ \langle b_1, b_2, ..., b_n \rangle = \langle a_{1}b_{1}, a_{2}b_{2}, ..., a_{n}b_{n} \rangle\] 
Nun ist der zu $C$ \textbf{duale Code} $C^{\perp}$ definiert als
\[C^{\perp} = \lbrace v \in \mathbb{F}_{p}^{n} \mid v \circ c = 0, \forall c \in C\rbrace\]
...
Wird ein dualer Code zu einem dualen Code eines linearen Codes $C$ gebildet, so ist dieser $C$, also gilt:
\[((C)^{\perp})^{\perp} = C\]
...
Ferner sei $\big( \langle h_11, h_12, ..., h_1n \rangle, \langle h_21, ..., h_2n \rangle, ..., \langle h_(n-k)1, ..., h_(n-k)n \rangle \big)$ eine Basis zu $C^{\perp}$.
\end{definition} 
