...
Nach einem Datenübertragungsvorgang wurde ein Tupel $r = \langle r_0, r_1, ..., r_{n-1}\rangle$ empfangen, das sich aus dem korrekten \textbf{Codeworttupel} $c = \langle c_0, c_1, ..., c_{n-1}\rangle$ und einem \textbf{Fehlertupel} $e = \langle e_0, e_1, ..., e_{n-1}\rangle$ zusammensetzt, wobei die Anzahl der Komponenten von $e$, die nicht null sind, nicht größer als $\lfloor\frac{n-k}{2}\rfloor$ sein darf, andernfalls ist es nicht möglich, mit diesem Dekodieralgorithmus das korrekte Codewort aus $r$ zu ermitteln. Im Folgenden bezeichne $k_1, ..., k_e$ die \textbf{Fehlerstellen} im empfangenen Wort, wobei $e < t$ gelten muss. $\alpha^{k_j}$ bezeichne die \textbf{Fehlstellennummer} und $e_{k_{j}}$ die \textbf{Fehlergröße} an der Stelle $k_j$. \\
...
\end{align*}
Da dieses Gleichungssystem jedoch angesichts der Exponenten nicht linear ist, lässt sich ohne großen Rechenaufwand keine triviale Lösung bestimmen, die jedoch für die Berechnung der Fehlerpositionen und Fehlergrößen notwendig wäre \parencite[vgl. ][S. 248]{Moon05} \parencite[vgl. ][S. 180f]{Huffman10} und \parencite[vgl. ][S. 148]{Hoffman1991}. \\\\
Stattdessen wird ein \textbf{Fehlerlokalisationspolynom} definiert:
\[\sigma(x) = \prod\limits_{j = 1}^{e}(1 - x \cdot \alpha_{k_{j}})\]
Jede inverse Fehlerstellennummer $\alpha_{k_{j}}^{-1}$ ist Wurzel dieses Polynoms \parencite[vgl. ][S. 185]{Roth06} \parencite[vgl. ][S. 180f]{Huffman10}.
