...
\end{theorem}
\begin{proof}
Die Eigenschaften folgen unmittelbar, so ist die komponentenweise Addition definiert als \[(v_0, v_2, ..., v_{n-1}) + (w_0, w_2, ..., w_{n-1}) = (v_0 + w_0, v_1 + w_1, ..., v_{n-1} + w_{n-1})\] und die Skalarmultiplikation als
\[k \circ (v_0, v_1, ..., v_{n-1}) = (k\circ v_0, k \circ v_1, ..., k \circ v_{n-1})\text{.}\] Damit ist leicht zu verifizieren, dass alle Forderungen erfüllt sind.
\end{proof}
\begin{definition}
