...
\item Der öffentliche Schlüssel ist nun $k_{pub} = S \times H \times P$
\end{itemize}
...
\label{NRDec}
\begin{algorithmic}[1]
\Require $S, H, P$, Geheimtextvektor $z = k_{pub} \times y^{T}$
\State $s \gets S^{-1} \times z$
\State $t \gets$ Ergebnis eines zum Code $C$ passenden Dekodierverfahrens auf $r$
\State $y \gets t \times (P^{T})^{-1}$ \\
\Return $y$
\end{algorithmic}
\end{algorithm} 
