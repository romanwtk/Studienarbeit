...
\begin{definition}
Ein Polynom $p(x) \in \mathbb{K}$ vom Grad (also dem höchsten Exponenten des Polynoms) $\deg (p) > 1$ wird als \textbf{irreduzibel} bezeichnet, falls es kein Polynom $q(k) \in \mathbb{K}$ mit $0 < \deg (q) <  \deg (p)$ gibt, das $p(x)$ teilt \parencite[vgl. ][S. 129]{Teschl}.
\end{definition}
Wird ein Polynomring nun durch reduzibles Polynom dividiert, also durch ein Polynom, dass sich selbst weiter dividieren lässt, ist nicht mehr sichergestellt, dass zu jedem Polynom ein multiplikatives Inverses existiert.
\begin{theorem}
Ein Ring $\mathbb{K}[x]_{m(x)}$ ist genau dann ein Körper, wenn $m(x)$ irreduzibel über $\mathbb{K}$ ist \parencite[vgl. ][S. 130]{Teschl}.
\end{theorem}
Ein Galois-Feld $\mathbb{F}_p$ kann auf diese Weise zu einem Körper $\mathbb{F}_{p^k}$ erweitert werden, in dem aus dem Polynomring $\mathbb{F}_{p}[x]$ ein irreduzibles Polynom $m(x)$ vom Grad $k$ ausgewählt wird. Dann ist 
\[\mathbb{F}_{p^k} \quad \Leftrightarrow \quad \mathbb{F}_{p}[x]_{m(x)}\]
\begin{example}
Typische Beispiele für endliche Körper sind:
\begin{itemize}
\item $\mathbb{F}_{2}$ bildet dem irreduziblen Polynom $x^2 + x + 1$ einen endlichen Körper $\mathbb{F}_{2^2} = \lbrace [0], [1], [x], [x+1]\rbrace$
