...
$\exists\, e \in G,  \: \forall \, a \in G: \quad a \circ e = e \circ a = a$
\item \textbf{Existenz inverser Elemente: }\\
$\forall\, a \in G,  \: \exists \, a^{-1} \in G: \quad a \circ a^{-1} = a^{-1} \circ a = e$
\end{enumerate}
Dann ist $(G, \circ)$ eine \textbf{Gruppe}. \\\\
Gilt für eine Gruppe zudem $\forall\, a, b \in G: \: a \circ b = b \circ a$, so ist sie \textbf{kommutativ}, auch bezeichnet als \textbf{abelsch} \parencite[vgl. ][S. 2]{Niederreiter97}.
\end{definition}
\begin{example}
...
\end{definition}
\begin{definition}
Ein kommutativer Divisionsring wird als \textbf{Feld} bezeichnet \parencite[vgl. ][S. 13]{Niederreiter97}. 
\end{definition}
...
\end{note}
\begin{example}
Ein klassisches Beispiel für einen auf diese Weise entstandenen Restklassenring ist \item $\mathbb{Z} / (p), p \in \mathbb{P}$, wobei $\mathbb{P}$ die Menge aller Primzahlen bezeichnet. Hier ist $(p)$ ein prinzipales Ideal, das durch $p$ generiert wird. Dieser Ring ist ein Feld, wie in \parencite{Niederreiter97} bewiesen wird und aufgrund der endlichen Anzahl an Elementen \textit{finit}. Die Elemente sind beschrieben durch 
\[\lbrace [0] = 0 + (n), [1] = 1 + (n), ... \rbrace\] 
Der Restklassenring $\mathbb{Z} / (3)$ enthält die Elemente $[0], [1] \text{ und } [2]$.
\end{example} 
Für \textit{Galois-Felder} wird nun lediglich noch eine Abbildung benötigt:
\begin{definition}
Eine Abbildung $\phi \colon R \to S$, wobei $R$ und $S$ Ringe sind, ist ein \textbf{Homomorphismus}, falls
\[ \phi (a + b) = \phi (a) + \phi (b) \quad \land \quad \phi (ab) = \phi (a) \phi (b)\]
